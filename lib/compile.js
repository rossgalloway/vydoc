...
var compileContract = function (dirPath, fileName, vyperCompilerPath, solidityCompilerPath) { return __awaiter(void 0, void 0, void 0, function () {
    var ext, cmd;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                cmd = '';
                if (ext === '.vy') {
                    cmd = "cd " + dirPath + " && cd .. && " + vyperCompilerPath + " -f combined_json " + path_1.join(dirPath, fileName);
                }
                else if (ext === '.sol') {
                    cmd = "cd " + dirPath + " && cd .. && " + solidityCompilerPath + " --combined-json abi,bin " + path_1.join(dirPath, fileName);
                }
                else {
                    throw new Error("Unsupported file extension: " + ext);
                }
                return [4 /*yield*/, customExec(cmd)];
...
var transformContract = function (data, name) {
    ...
    var methodsWithAbi = Object.keys(methods).reduce(function (acc, methodSignature) {
        var _a;
        var methodName = methodSignature.replace(/\(.*/, '');
        var abiMethod = abiMethods[methodName];
        var method = __assign(__assign({}, methods[methodSignature]), (abiMethod ? abiMethod : {}));
        if (abiMethod && abiMethod.outputs && abiMethod.outputs.length > 0) {
            method.returns = abiMethod.outputs.map(function (output, index) {
                var name = output.name || "_output" + index;
        ...
        return __assign(__assign({}, acc), (_a = {}, _a[methodSignature] = method, _a));
    }, {});
    var r = {
...
var compileContracts = function (dirPath, fileNames, vyperCompilerPath, solidityCompilerPath) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
                                ...
                                return __generator(this, function (_b) {
                                    switch (_b.label) {
                                        case 0:
                                            ...
                                            return [4 /*yield*/, exports.compileContract(dirPath, fileName, vyperCompilerPath, solidityCompilerPath)
                                                    .then(JSON.parse)
